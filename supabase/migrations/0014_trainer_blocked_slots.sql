-- KidsConnect — a coach marks which of the weekly slots they're actually free for,
-- on top of whatever's already booked from a real enrollment. Both layers show as
-- "taken" to a parent picking a slot, and both are enforced server-side, not just drawn
-- differently in the UI.

create table public.trainer_blocked_slots (
  trainer_id uuid not null references public.trainer_profiles(user_id) on delete cascade,
  weekday    smallint not null check (weekday between 0 and 6),
  time       time not null,
  created_at timestamptz not null default now(),
  primary key (trainer_id, weekday, time)
);

alter table public.trainer_blocked_slots enable row level security;

-- Readable by anyone signed in — same "shop window" rule as trainer_profiles, since a
-- parent has to see it before they can pick around it.
create policy trainer_blocked_slots_read on public.trainer_blocked_slots
  for select to authenticated using (true);

create policy trainer_blocked_slots_owner_insert on public.trainer_blocked_slots
  for insert to authenticated with check (trainer_id = auth.uid());

create policy trainer_blocked_slots_owner_delete on public.trainer_blocked_slots
  for delete to authenticated using (trainer_id = auth.uid());

grant select, insert, delete on public.trainer_blocked_slots to authenticated;

-- ---------------------------------------------------------------- enforce in send_enquiry
